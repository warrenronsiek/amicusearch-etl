...
object GenericCourtlisterReader {
  ...
  def apply(path: String, env: AppParams.Environment.Value, schema: StructType)(implicit spark: SparkSession): Unit => DataFrame = {
    _ => env match {
        ...
        case AppParams.Environment.dev => spark.read.schema(schema).option("header", "true").parquet(path)
        case AppParams.Environment.prod => spark.read.schema(schema).option("header", "true").parquet(path)
      }
  }
}
